...
struct RecipeDetailCookingView: View {
    ...
    /// The first step not yet ticked off — the one the marker fills in, so there
    /// is always exactly one "you are here" on screen. Nil once everything is
    /// done, which leaves no step highlighted rather than pointing at the last.
    ...
    /// The step number, filled amber on the step you're up to and plain
    /// everywhere else.
    ///
    /// The frame is the same either way so the text column never shifts as the
    /// marker moves down the list, and the circle is drawn in the background so
    /// the digit keeps the `.firstTextBaseline` alignment the row is built on.
