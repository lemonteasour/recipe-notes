...
/// `RecipeBBApp.init()`, where appearance proxies are still read in time for
/// the first bar.
@MainActor
enum RoundedChromeAppearance {
    ...
    /// A no-op if the system draws the tab bar itself rather than through
    /// `UITabBar`, which is fine — nothing breaks either way.
